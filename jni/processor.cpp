...
bool throwJavaException(JNIEnv *env, std::string method_name,
		std::string exception_msg, int errorCode = 0) {
	char buf[8];
...
void cb(cl_program p, void* data) {
	...
	clGetProgramInfo(p, CL_PROGRAM_DEVICES, sizeof(cl_device_id), (void*) devid,
			NULL);
	char bug[65536];
	clGetProgramBuildInfo(p, devid[0], CL_PROGRAM_BUILD_LOG,
			65536 * sizeof(char), bug, NULL);
	//	clReleaseProgram(p);
	LOGE("Build log \n %s\n", bug);

	cl_build_status status;
	clGetProgramBuildInfo(p, devid[0], CL_PROGRAM_BUILD_STATUS,
			sizeof(cl_build_status), &status, NULL);
	LOGE("Build status \n %d\n", status);

	char option[65536];
	clGetProgramBuildInfo(p, devid[0], CL_PROGRAM_BUILD_OPTIONS,
			65536 * sizeof(char), &option, NULL);
	LOGE("Build option \n %s\n", option);
...
void process(uint32_t* outL1, uint32_t* outL2, uint32_t* outL3, uint8_t* in,
		int w, int h) {
	static int first = 1;
	...
	try {
		//LOGI("@process \n");
		gQueue.enqueueWriteBuffer(*bufferIn, CL_TRUE, 0,
				w * h * sizeof(cl_uchar4), in);
		////////////////////////////////////
		...
		gNV21Kernel.setArg(4, w*h);
		gQueue.enqueueNDRangeKernel(gNV21Kernel, cl::NullRange,
				cl::NDRange((int) ceil((float) w / 16.0f) * 16,
						(int) ceil((float) h / 16.0f) * 16),
		...
		gQueue.enqueueNDRangeKernel(gDownSamplerXKernel, cl::NullRange,
				cl::NDRange((int) ceil((float) w / nbthreadx) * nbthreadx,
						(int) ceil((float) h / nbthready) * nbthready),
						cl::NDRange(nbthreadx, nbthready), NULL, NULL);
		...
		gQueue.enqueueNDRangeKernel(gDownSamplerYKernel, cl::NullRange,
				cl::NDRange((int) ceil((float) w / 2 / nbthreadx) * nbthreadx,
						(int) ceil((float) h / 2 / nbthready) * nbthready),
						cl::NDRange(nbthreadx, nbthready), NULL, NULL);
	...
	try {
		...
		gQueue.enqueueNDRangeKernel(gDownSamplerXKernel, cl::NullRange,
				cl::NDRange((int) ceil((float) w / 4 / nbthreadx) * nbthreadx,
						(int) ceil((float) h / 4 / nbthready) * nbthready),
						cl::NDRange(nbthreadx, nbthready), NULL, NULL);
...
JNIEXPORT void JNICALL runfilter(JNIEnv *env, jclass clazz, jobject L1Bmp,
		jobject L2Bmp, jobject L3Bmp, jbyteArray inData, jint width,
		jint height) {
	...
	if (AndroidBitmap_getInfo(env, L1Bmp, &bmpInfoL1) < 0) {
		throwJavaException(env, "gaussianBlur",
				"Error retrieving bitmap meta data");
		return;
	}
	...
	if (inPtr == NULL) {
		throwJavaException(env, "gaussianBlur",
				"NV21 byte stream getPointer returned NULL");
		return;
	}

	process(bmpContentL1, bmpContentL2, bmpContentL3, (uint8_t*) inPtr, width,
			height);
...
JNIEXPORT jboolean JNICALL compileKernels(JNIEnv *env, jclass clazz, int w,	int h) {
	...
	try {
		...
		LOGI("@compileKernels \n");
		int src_length = 0;
		const char* src = file_contents(
				"/data/data/com.example/app_execdir/kernels.cl",
				&src_length);
		cl::Program::Sources sources(1, std::make_pair(src, src_length + 1));
		program = new cl::Program(gContext, sources, &err);
		program->build(devices);
		//		while (program->getBuildInfo<CL_PROGRAM_BUILD_STATUS>(devices[0])
		//				!= CL_BUILD_SUCCESS);
		...
		bufferOut = new cl::Buffer(gContext, CL_MEM_READ_WRITE,
				w * h * sizeof(cl_uchar4));
		bufferOut2 = new cl::Buffer(gContext, CL_MEM_READ_WRITE,
				w * h * sizeof(cl_uchar4));
		bufferIn = new cl::Buffer(gContext, CL_MEM_READ_WRITE,
				w * h * sizeof(cl_uchar4));
		...
		LOGI("@compileKernels error\n");

		cl::STRING_CLASS buildstatus;
		program->getBuildInfo(devices[0],CL_PROGRAM_BUILD_STATUS, &buildstatus);
		cl::STRING_CLASS buildlog;
		program->getBuildInfo(devices[0],CL_PROGRAM_BUILD_LOG, &buildlog);

		LOGE("@compileKernels error %s \n",buildlog.c_str());
		LOGE("@compileKernels error %s \n",buildlog.c_str());
...
{ { "compileKernels", "(II)Z", (void *) compileKernels },
		{ "runfilter",
				"(Landroid/graphics/Bitmap;Landroid/graphics/Bitmap;Landroid/graphics/Bitmap;[BII)V",
				(void *) runfilter }, { "getSpeed", "()I",
						(void *) getSpeed } };
...
JNIEXPORT jint JNI_OnLoad(JavaVM* pVM, void* reserved) {
	...
	LOGW("JNI_OnLoad\n");
	if ((pVM->GetEnv((void **) (&env), JNI_VERSION_1_6)) != JNI_OK) { //abort();
	}

	LOGW("JNI_OnLoad\n");
	jclass LiveFeatureActivity = env->FindClass("com/example/MainActivity");
	if (LiveFeatureActivity == NULL)
		abort();
	env->RegisterNatives(LiveFeatureActivity,
			gMethodRegistryLiveFeatureActivity, 3);
